...
pub const RoutingTable = struct {
    ...
    pub fn closestTo(self: *const RoutingTable, dst: []ID, public_key: PublicKey) usize {
        ...
        const bucket_index = clz(xor(self.public_key, public_key));
        if (!std.mem.eql(u8, &self.public_key, &public_key)) {
...
fn AutoStaticHashMap(comptime K: type, comptime V: type, comptime capacity: comptime_int) type {
...
fn StaticHashMap(comptime K: type, comptime V: type, comptime Context: type, comptime capacity: usize) type {
    ...
    return struct {
        const empty_hash = std.math.maxInt(u64);
        ...
        pub fn getOrPutAssumeCapacityContext(self: *Self, key: K, ctx: Context) Self.GetOrPutResult {
            ...
            std.debug.assert(it.hash != Self.empty_hash);
        ...
        pub fn getContext(self: *Self, key: K, ctx: Context) ?V {
            ...
            std.debug.assert(hash != Self.empty_hash);
